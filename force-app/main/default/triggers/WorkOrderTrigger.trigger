trigger WorkOrderTrigger on BOATBUILDING__Work_Order__c (before insert, after insert, before update, after update, after delete, before delete) {
    Boolean isOff = BOATBUILDING__TriggerSetting__c.getOrgDefaults().BOATBUILDING__WorkOrderTriggerHandlerChecBox__c;
    if(isOff==true){
        if(trigger.isAfter && trigger.isUpdate) {
            WorkOrderTriggerHandler.updateInventoryQuantity(trigger.new, trigger.oldMap);
        }
        if(trigger.isBefore) {
            if(trigger.isDelete) {
                WorkOrderTriggerHandler.onBeforeDelete(trigger.oldMap);
            }
            if(trigger.isInsert) {
                ...
                for(Work_Order__c objWO: trigger.new) {
                    ...
                    if(objWO.BOATBUILDING__Account__c != null && objWO.RecordTypeId == Schema.SObjectType.BOATBUILDING__Work_Order__c.getRecordTypeInfosByName().get('Work Request').getRecordTypeId()) {
                        setAccId.add(objWO.BOATBUILDING__Account__c);
                    }
                } 
                if(!setAccId.isEmpty()) {
                    Map<Id, Account> mapAcc = new Map<Id, Account>([Select Id, BOATBUILDING__Store_Location__c From Account Where Id IN: setAccId ]);
                    for(Work_Order__c objWO: trigger.new) {
                        if(objWO.BOATBUILDING__Account__c != null && objWO.RecordTypeId == Schema.SObjectType.BOATBUILDING__Work_Order__c.getRecordTypeInfosByName().get('Work Request').getRecordTypeId()
                           && mapAcc.containsKey(objWO.BOATBUILDING__Account__c) && mapAcc.get(objWO.BOATBUILDING__Account__c).BOATBUILDING__Store_Location__c != null) {
                               objWO.BOATBUILDING__Store_Location__c = mapAcc.get(objWO.BOATBUILDING__Account__c).BOATBUILDING__Store_Location__c;
                           }
                    } 
                }
            }
        ...
        if(trigger.isAfter) {
            if(trigger.isInsert || trigger.isUpdate) {
                ...
                if(!setAccId.isEmpty()) {
                    List<Account> lstAcc = [Select Id, Service_score__c, (Select id From Work_Orders__r Where Status__c = 'Closed')
                ...
                List<BOATBUILDING__Dealership_Form__c> lstDFUpdate = new List<BOATBUILDING__Dealership_Form__c>();
                for(BOATBUILDING__Dealership_Form__c objDF: lstDF) {
                    
                    if(!Approval.isLocked(objDF.Id)) {
                        ...
                        lstDFUpdate.add(objDF);
                    }
                    
                }
                if(!lstDFUpdate.isEmpty()) { 
                    update lstDFUpdate;
                }
            }
            if(trigger.isUpdate) {
                            ...
                            if(objWO.RecordtypeId == reWOId) { 
                                if(objWO.Store_Location__c != null) {
                                    ...
                                    if(objBoatBuilderUtil.isCanadianTax()) {
                                        trigger.newMap.get(objWO.Id).BOATBUILDING__GST__c = objBoatBuilderUtil.getGST();  
                                        trigger.newMap.get(objWO.Id).BOATBUILDING__GST_D__c = (taxCalAmt * objBoatBuilderUtil.getGST())/100;
                                        trigger.newMap.get(objWO.Id).BOATBUILDING__PST__c = objBoatBuilderUtil.getPST();
                                        trigger.newMap.get(objWO.Id).BOATBUILDING__PST_D__c = (taxCalAmt * objBoatBuilderUtil.getPST())/100;
                                    }
                                } else {  
        ...
        if(trigger.isAfter) {
            if(trigger.isUpdate || trigger.isInsert) {
                RiggingHandler.onAfterInsertWO(trigger.newMap, trigger.oldMap);
            }  
            if(trigger.isDelete)
            {
                System.debug('>>>>New: '+trigger.new);
                System.debug('>>>>Old: '+trigger.old);
                WorkOrderTriggerHandler.onAfterDelete(trigger.old);
            }         
        }
    }
}